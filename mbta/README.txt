...
1. Elements correctly implemented: My geolocator is working and renders a marker with, as its icon, a green symbol that I created in the GNU manipulator for this lab. I correctly calculated the Haversine formula for the nearest Red Line Station and outputted that to the infowindow. I rendered a polyline from the geolocation to the nearest Red Line station. I added infowindows on all Red Line markers which display information for each station's scheduled trains.

Elements not correctly implemented: All elements are now correctly implemented for the program.
